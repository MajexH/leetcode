...
// 
//
// Related Topics 栈 队列 数组 模拟 👍 101 👎 0
...
public class NumberOfStudentsUnableToEatLunch1700 {
    public static void main(String[] args) {
        Solution solution = new NumberOfStudentsUnableToEatLunch1700().new Solution();
        ...
        System.out.println(solution.countStudents(new int[]{1, 1}, new int[]{0, 1}));
    }
    ...
    class Solution {
        public int countStudents(int[] students, int[] sandwiches) {
            ...
            // 已经吃完离开
            boolean[] departures = new boolean[students.length];
            ...
            while (j < sandwiches.length) {
                ...
                // 学生已经离开 或者 不能吃
                if (departures[i] || students[i] != sandwiches[j]) {
                    i = (i + 1) % students.length;
                    move = true;
                    continue;
                }

                // 学生和三明治相等 那么可以转移到下一个数据上
                departures[i] = true;
                i = (i + 1) % students.length;
                start = i;
                j++;
