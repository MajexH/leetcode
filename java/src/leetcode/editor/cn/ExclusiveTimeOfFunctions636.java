...
public class ExclusiveTimeOfFunctions636 {
    public static void main(String[] args) {
        Solution solution = new ExclusiveTimeOfFunctions636().new Solution();
        ...
        System.out.println(Arrays.toString(solution.exclusiveTime(1, Arrays.asList("0:start:0","0:end:0"))));
        System.out.println(Arrays.toString(solution.exclusiveTime(1, Arrays.asList("0:start:0","0:start:1","0:start:2","0:end:3","0:end:4","0:end:5"))));
        System.out.println(Arrays.toString(solution.exclusiveTime(3, Arrays.asList("0:start:0","0:end:0","1:start:1","1:end:1","2:start:2","2:end:2","2:start:3","2:end:3"))));
    }
    ...
    class Solution {
        ...
        public int[] exclusiveTime(int n, List<String> logs) {
            ...
            Deque<Log> stacks = new LinkedList<>();

            for (String log : logs) {
                String[] logInfo = log.split(":");

                int curN = Integer.parseInt(logInfo[0]);
                ...
                switch (logInfo[1]) {
                    case "start":
                        // 相当于前一个函数停止执行
                        if (!stacks.isEmpty()) {
                            Log pre = stacks.getLast();
                            // 累计前一个函数的时间
                            res[pre.n] += timeStamp - pre.start;
                        }
                        stacks.addLast(new Log(curN, timeStamp));
                        break;
                    case "end":
                        // 相当于前一个函数再重新开始执行 其开始时间重置
                        // 一定不为空
                        assert !stacks.isEmpty();
                        Log pre = stacks.removeLast();
                        // 当前 n 执行累计的时间
                        res[pre.n] += timeStamp - pre.start + 1;

                        // 更新调用它的函数的开始时间为当前时刻
                        if (!stacks.isEmpty()) {
                            stacks.getLast().start = timeStamp + 1;
                        }
                }
            }
